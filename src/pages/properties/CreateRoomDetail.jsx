...
export default function CreateRoomDetail() {
  ...
  return <form className="create-room-page" onSubmit={submit}><div className="add-room-page">
    <div className="add-room-header">
      <div>
        <h1>Add Room</h1>
        <p>{property.title} · Room {roomCount + 1} of {property.totalBedrooms}</p>
      </div>
      <Link className="back-button" to={`/owner/properties/${propertyId}`}>← Back to the Unit</Link>
    </div>
    {error && <p className="mx-auto mb-5 max-w-7xl rounded-xl bg-red-50 p-3 text-danger" role="alert">{error}</p>}
    <section className="room-section">
      ...
      <div className="section-form two-columns">
        <div className="form-group">
          <label>Room Title <span>*</span></label>
          <input
            name="roomName"
            maxLength="100"
            value={form.roomName}
            onChange={update}
            placeholder="Example: Room A1" />
          <div className="form-helper">
            <span>Name the room so tenants can distinguish it.</span>
            <span>{form.roomName.length} / 100</span>
          </div>
        </div>
        <div className="form-group">
          <label>Room Description <small>(Optional)</small></label>
          <textarea
            name="description"
            maxLength="500"
        ...
        <div className="form-group">
          ...
          <div className="price-input">
            ...
            <input
              ...
              onChange={update} />
            <div className="price-suffix">/ Month
            </div>
          </div>
        </div>
        <div className="form-group">
          <label>Room Status <span>*</span></label>
          <div className="room-status-options">
            {[["AVAILABLE", "Available", "This room is available for rent"], ["RENTED", "Rented", "This room is currently occupied"]].map(([value, title, copy]) =>
              <button
                key={value}
                type="button"
                className={`status-card ${form.status === value ? "active" : ""}`}
                onClick={() => setForm((current) => ({ ...current, status: value }))}
              >
                <div>
                  <div className="status-title">
                    <span className={`status-dot ${value.toLowerCase()}`} /><strong>{title}</strong>
                  </div>
                  <p>{copy}</p>
                </div>
                {form.status === value && <CheckCircle2 className="status-check" size={24} />}
              </button>
            )}
          </div>
        </div>
    ...
    <section className="room-section">
      ...
      <div className="form-group guest-group">
        <label>Max. Resident <small>(Optional)</small></label>
    ...
    <section className="room-section image-section">
      ...
      <div className="section-divider" />
      <p className="image-description">Upload 1 JPG, PNG, WebP or GIF image, maximum 5 MB.</p>
      <label className="upload-box">
        <UploadCloud size={36} className="upload-icon" />
        <strong>Click to choose a room photo</strong>
        <span>{images.length} / 1 selected</span>
        <input
          type="file"
          accept="image/*"
          className="sr-only"
        ...
        <div className="preview-grid">
          {previews.map(({ file, url }, index) =>
            <div className="relative" key={`${file.name}-${file.lastModified}`}>
              <img src={url} alt="" className="h-36 w-full rounded-lg object-cover" />
              {index === 0 && <span className="absolute bottom-2 left-2 rounded-full bg-forest px-2 py-1 text-xs text-white">Cover</span>}
              <button
                type="button"
                aria-label={`Remove ${file.name}`}
                onClick={() => setImages((current) => current.filter((_, itemIndex) => itemIndex !== index))}
                className="absolute right-2 top-2 grid size-8 place-items-center rounded-full border-0 bg-white text-danger">
                <Trash2 size={15} />
              </button>
            </div>
          )}</div>
