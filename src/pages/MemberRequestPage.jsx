...
export default function MemberRequestPage() {
  ...
  const {
    getMemberName, getMemberAvatar, triggerToast,
    post, requests, acceptedMembers, loading, processingRequestId, toast,
    maxCapacity, pendingRequests, pendingCount, acceptedCount, rejectedCount,
    isFull, spotsRemaining, capacityPercentage, handleRequestAction,
  } = useMemberRequests(postId);

  if (loading)
    return (
      <div className="p-8 text-muted-copy">Loading community requests...</div>
    );
  if (!post)
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
          <div className="flex flex-col gap-1.5">
            ...
            <p className="text-sm md:text-base text-muted-copy">
              Review applicant profiles, approve join requests, and balance your
              room allocation.
            </p>
          </div>
              ...
              <div>
                <div className="flex flex-wrap items-center gap-2.5 mb-2.5">
                  <span className="px-3 py-1 rounded-full bg-[#eedcd4] text-[#695c56] text-xs font-semibold">
                    {post.property?.propertyType || "Property"} ·{" "}
                    {post.property?.address?.province || "Location unavailable"}
                  </span>
                ...
                <p className="text-sm md:text-[15px] text-muted-copy line-clamp-2 leading-relaxed mb-4">
                  {parsePostGender(post.description).cleanDescription ||
              ...
              <div className="flex flex-wrap items-center justify-between gap-4 pt-4 border-t border-[#f1eee4] mt-auto">
                <div className="flex items-baseline gap-1.5">
                  <span className="font-serif text-2xl font-bold text-[#4f614d]">
                    ฿{Number(post.property?.monthlyRent || 0).toLocaleString()}
                  </span>
                  <span className="text-muted-copy text-xs sm:text-sm font-medium">
                    / month
                  </span>
                </div>
                <div className="flex items-center gap-3">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6 mb-8">
          ...
          <div className="p-5 rounded-2xl bg-white border border-[#e1e5dd] shadow-xs flex flex-col justify-between">
            ...
            <div className="mt-4 flex items-baseline justify-between">
              <span className="font-serif text-3xl font-bold text-[#1c1c16]">
                {pendingCount}
              </span>
          ...
          <div className="p-5 rounded-2xl bg-white border border-[#e1e5dd] shadow-xs flex flex-col justify-between">
            ...
            <div className="mt-4 flex items-baseline justify-between">
              ...
              <span className="text-muted-copy text-xs font-medium">
                Archived
              </span>
            </div>
          </div>
              ...
              <div className="flex items-baseline justify-between mb-1.5">
                ...
                <span
                  className={`text-xs font-bold ${
                    isFull ? "text-[#4f614d]" : "text-terracotta"
                  }`}
                >
                          ...
                          {applicant.status === "ACCEPTED" ? (
                            ...
                            <>
                              <button
                                type="button"
                                onClick={() =>
                                  handleRequestAction(applicant, "REJECT")
                                }
                              ...
                              <button
                                type="button"
                                onClick={() =>
                                  handleRequestAction(applicant, "ACCEPT")
                                }
                                disabled={
                                  isFull || processingRequestId === applicant.id
                                }
                              ...
                              >
                                ...
                                {processingRequestId === applicant.id
                                  ? "Saving..."
                                  : "Accept"}
                              </button>
                            </>
            ...
            >
              ...
              <div>
                <h5 className="text-sm font-bold text-[#1c1c16]">
                  {isFull
                    ? "Group Full - Listing Locked"
                    : `${spotsRemaining} Available Spot Remaining`}
                </h5>
                ...
                {acceptedMembers.map((member) => (
                  ...
                  >
                    ...
                    <div className="pt-2 border-t border-[#e8e7dc] flex justify-between items-center text-xs text-muted-copy">
                      <span>Room:</span>
                      <span className="text-[#1c1c16] font-semibold">
                        {member.room}
                      </span>
                    </div>
                  </div>
            ...
            <div className="bg-white p-6 rounded-2xl border border-[#e1e5dd] shadow-sm">
              ...
              <div>
                <button
                  type="button"
                  onClick={() =>
                    triggerToast("Opening Unit Rooms management...")
                  }
            ...
            <div className="bg-white p-6 rounded-2xl border border-[#e1e5dd] shadow-sm">
              ...
              <ul className="flex flex-col gap-2.5 text-xs text-muted-copy leading-relaxed">
                <li className="flex items-start gap-2">
                  ...
                  <span>
                    Check alignment on working hours and morning schedule.
                  </span>
                </li>
                <li className="flex items-start gap-2">
                  <CheckCircle2 className="w-4 h-4 text-[#4f614d] shrink-0 mt-0.5" />
                  <span>
                    Confirm dietary, pet, and guest policies with existing
                    roommates.
                  </span>
                </li>
                <li className="flex items-start gap-2">
                  <CheckCircle2 className="w-4 h-4 text-[#4f614d] shrink-0 mt-0.5" />
                  <span>
                    Schedule a quick 5-min virtual meetup before finalizing
                    lease.
                  </span>
                </li>
              </ul>
