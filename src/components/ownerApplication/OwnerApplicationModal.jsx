...
import {
  resubmitOwnerApplication,
  submitOwnerApplication,
} from "../../services/ownerApplicationService.js";
...
const STATUS_CONTENT = {
  PENDING: {
    label: "Under review",
    description:
      "Your documents are being reviewed. You cannot change them during review.",
  },
  NEED_MORE_DOCUMENTS: {
    label: "Action required",
    description:
      "Additional documents are required before your application can be reviewed.",
  },
  REJECTED: {
    label: "Rejected",
    description: "Your owner application was not approved.",
  },
  APPROVED: {
    label: "Approved",
    description: "Your owner application has been approved.",
  },
};
...
const OwnerApplicationModal = ({
  ...
  const handleFiles = (event) => {
    ...
    if (invalid)
      return setError(`${invalid.name} is not a JPEG, PNG, WebP, or PDF file.`);
    ...
    try {
      ...
      setError(
        requestError.response?.status === 409
          ? "You already have an owner application."
  ...
  const handleResubmit = async (event) => {
    ...
    if (!files.length)
      return setError(
        isAppending
          ? "Please select at least one additional document."
          : "Please select at least one replacement document.",
      );
    setIsSubmitting(true);
    setError("");
    setSuccess("");
    try {
      ...
      setFiles([]);
      setSuccess(
        "Documents submitted successfully. Your application is now under review.",
      );
      await onSubmitted?.();
    } catch (requestError) {
      setError(
        getBackendMessage(requestError) ||
          "Unable to resubmit your documents. Please try again.",
      );
    } finally {
      setIsSubmitting(false);
    }
  ...
  const statusContent =
    ...
    (STATUS_CONTENT[application.status] || {
      ...
      description: "This application has an unknown status.",
    });
    ...
    <div className="owner-application-overlay" onMouseDown={closeModal}>
      ...
      >
        <header className="owner-application-header">
          <div>
            <h2 id="owner-application-title">
              {application ? "Owner application" : "List your property"}
            </h2>
            <p>
              {application
                ? "Application details and review status."
                : "Submit documents to apply for an owner account."}
            </p>
          </div>
          <button
            type="button"
        ...
        <div className="owner-application-content">
          {application ? (
            <div className="owner-application-status-details">
              <div
                className={`owner-application-status-card status-${application.status.toLowerCase().replaceAll("_", "-")}`}
              >
                <span>Current status</span>
                <strong>{statusContent.label}</strong>
                <p>{statusContent.description}</p>
              </div>
              {(application.rejectReason || application.adminMessage) && (
                <div className="owner-application-admin-message">
                  <strong>Admin message</strong>
                  <p>{application.rejectReason || application.adminMessage}</p>
                </div>
              )}
              <dl className="owner-application-metadata">
                <div>
                  <dt>Submitted</dt>
                ...
                <div>
                  <dt>Phone number</dt>
                  <dd>{application.phone || "Not available"}</dd>
                </div>
              </dl>
              <section className="owner-application-document-section">
                <h3>
                  {["NEED_MORE_DOCUMENTS", "REJECTED"].includes(
                    application.status,
                  )
                ...
                {application.documents?.length ? (
                  <ul className="owner-application-files">
                  ...
                  <p className="owner-application-empty">
              ...
              {["NEED_MORE_DOCUMENTS", "REJECTED"].includes(
                ...
                <form
                  className="owner-application-resubmit"
                  onSubmit={handleResubmit}
                  noValidate
                >
                  <p>
                    {isAppending
                      ? `Add only the documents requested by the review team. Your ${application.documents?.length || 0} previous document${application.documents?.length === 1 ? "" : "s"} will be kept.`
                      : "Select the complete corrected document set. A successful resubmission will replace all previously submitted documents."}
                  </p>
                  <label
                    className="owner-application-upload"
                    htmlFor="owner-resubmit-documents"
                  >
                    <Upload size={28} />
                    <strong>
                      {isAppending
                    ...
                    <span>
                      JPEG, PNG, WebP, or PDF · Up to {maximumNewFiles} file
                      {maximumNewFiles === 1 ? "" : "s"} · 5 MB each
                    </span>
                  </label>
                  <input
                    id="owner-resubmit-documents"
                    className="owner-application-file-input"
                    type="file"
                    multiple
                    accept={ALLOWED_TYPES.join(",")}
                    onChange={handleFiles}
                    disabled={isSubmitting || maximumNewFiles === 0}
                  />
                  {maximumNewFiles === 0 && (
                    <p className="owner-application-error">
                      This application already has the maximum of 5 documents.
                      Contact support if another file was requested.
                    </p>
                  )}
                  {!!files.length && (
                    <ul
              ...
              {success && (
                <p className="owner-application-success-message" role="status">
                  {success}
                </p>
              )}
              <button
                type="button"
                className="owner-application-done"
                onClick={closeModal}
              >
                Done
              </button>
